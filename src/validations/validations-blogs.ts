...
export const validationCreateOrUpdateBlog = [
    ...
    body('name').isString().trim().isLength({min: 1, max: 15}).withMessage('name must not be empty and name length min 1 max 15'),
    body('description').notEmpty().withMessage('description is required'),
    body('description').isString().trim().isLength({min: 1, max: 500}).withMessage('description must not be empty and description length min 1 max 15'),
